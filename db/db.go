...
// DB contains the unterlying BoltDB handle
type DB struct {
	Handle *bolt.DB
}

// New creates a new DB Handle.
...
func (db *DB) Store(bucket, key, value string) error {
...
// Delete a key from a BoltDB bucket. Deleting an non-existing key will return nil.
func (db *DB) Delete(bucket, key string) error {
	...
	err := db.Handle.Update(func(tx *bolt.Tx) error {
		...
		if err != nil {
			return fmt.Errorf("Could not delete key from bucket %q: %v", bucket, err)
		}
...
func (db *DB) Fetch(bucket, key string) (string, error) {
...
// FetchAll will fetch every key from a BoltDB bucket
func (db *DB) FetchAll(bucket string) ([]string, error) {
	var vals []string
